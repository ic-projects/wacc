...
func (v *CodeGenerator) visitPointerNewNode(node *ast.PointerNewNode) {
	register := v.getFreeRegister()
...
/**************** POINTER DEREFERNCE NODE ****************/

func (v *CodeGenerator) visitPointerDereferenceNode(
	node *ast.PointerDereferenceNode,
) {
	ast.Walk(v, node.Ident)
	register := v.returnRegisters.Peek()
